...
浏览器端 React 应用使用 Appwrite Web SDK 时，正常情况下只需要 `endpoint` 和 `projectId`，不需要 API Key。

只有在以下场景才需要 API Key：
- 你自己编写服务端接口，使用 Appwrite Server SDK 代管上传或写库
- 你需要执行管理级操作，且这些操作不能直接暴露给浏览器

如果你确实在服务端使用 API Key，才需要创建并授予以下权限：
...
| 权限不足 | 平台、集合或存储桶权限不足 | 检查 Web 平台、Collection/Bucket 权限 |
| 文件过大 | 上传文件超过大小限制 | 检查存储桶配置的文件大小限制 |
| 网络错误 | 无法连接到 Appwrite | 检查 URL、Cloudflare Access/WAF、CORS 和网络连接 |
...
4. **启用 CORS**
  ...
  - 如果前面还有 Cloudflare Access、Zero Trust 或 WAF，也要确保 `/v1/*` 不被拦截
