...
function SubmitModal({ onClose }: { onClose: () => void }) {
  ...
  const bucketNameMap: Record<string, string> = {
    '69df42a60004a562ba07': '明信片图片桶',
    '69df430100088d621422': '演示文稿桶',
    '69df4311003df133de23': '视频桶',
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.category === 'video') {
      ...
      // Prevent accidental non-link text submissions for video category.
      if (!/^https?:\/\//i.test(normalizedVideoUrl)) {
        setError('视频类别请填写完整网盘链接（需以 http:// 或 https:// 开头）');
        return;
      }
    ...
    try {
      ...
      setUploadReceipt({
        category: formData.category,
        bucketId: (result?.bucketId as string | null) ?? null,
        storedFileId: ((result?.file as { $id?: string } | null)?.$id ?? null),
        storedFileName: ((result?.file as { name?: string } | null)?.name ?? null),
        documentId: ((result?.entry as { $id?: string } | null)?.$id ?? null),
      });
                ...
                {formData.category === 'video' ? (
                  ...
                  <div className="space-y-2">
                    <div className="p-4 rounded-xl bg-blue-50 border border-blue-200 text-sm text-blue-700">
                      ...
                      <p className="text-xs">支持百度云盘、夸克网盘等。仅写入数据表字段 <strong>videoUrl</strong>，不会上传到存储桶。</p>
                    </div>
                    <input
                      type="text"
                      name="videoUrl"
          ...
          {step === 3 && (
            <div className="py-24 flex flex-col items-center justify-center space-y-6 text-center">
              ...
              <div className="space-y-3">
                ...
                {uploadReceipt && uploadReceipt.category !== 'video' && (
                  <div className="mt-4 rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-left text-sm text-zinc-700 space-y-1">
                    <p>存储桶：{uploadReceipt.bucketId ? (bucketNameMap[uploadReceipt.bucketId] || uploadReceipt.bucketId) : '未知'}</p>
                    <p>文件名：{uploadReceipt.storedFileName || '未知'}</p>
                    <p>文件ID：{uploadReceipt.storedFileId || '未知'}</p>
                    <p>文档ID：{uploadReceipt.documentId || '未知'}</p>
                  </div>
                )}
                {uploadReceipt && uploadReceipt.category === 'video' && (
                  <div className="mt-4 rounded-xl border border-zinc-200 bg-zinc-50 p-4 text-left text-sm text-zinc-700 space-y-1">
