...
export async function submitEntry({
  ...
  try {
    ...
    const uploadedFileId = (uploadedFile as { $id?: string })?.$id;
    if (!uploadedFileId) {
      throw new Error('文件上传返回异常：未获取到文件 ID，请重试');
    }
    ...
    // File-based categories should also persist a submission row.
    // Keep document payload conservative to avoid schema regressions.
    const entry = await databases!.createDocument(
      config.databaseId,
      config.collectionId,
      entryId,
    ...
    console.info('[Appwrite] File entry created:', {
      entryId: entry.$id,
      category,
