...
const CATEGORY_BUCKETS: Record<Exclude<CategoryKey, 'video'>, string> = {
...
function SubmitModal({ onClose }: { onClose: () => void }) {
  ...
  const predictedStoredFileName =
    file && formData.studentId.trim()
      ? `${formData.studentId.trim()}_${formData.category}.${file.name.split('.').pop() || 'bin'}`
      : null;
                ...
                {formData.category === 'video' ? (
                  ...
                  <div className="space-y-3">
                    <div className="rounded-xl border border-orange-200 bg-orange-50 px-4 py-3 text-sm text-orange-700">
                      <p className="font-semibold">当前上传目标</p>
                      <p className="mt-1 text-xs">存储桶：{CATEGORY_BUCKETS[formData.category as 'postcard' | 'presentation']}</p>
                      {predictedStoredFileName && (
                        <p className="mt-1 text-xs">入桶文件名：{predictedStoredFileName}</p>
                      )}
                    </div>
                    <div
