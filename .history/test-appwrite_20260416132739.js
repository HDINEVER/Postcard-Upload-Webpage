...
fetch(config.url + '/health', { redirect: 'manual' })
  .then(response => {
    if (response.status >= 300 && response.status < 400) {
      const location = response.headers.get('location') || '';
      console.log('⚠️  健康检查被重定向');
      console.log(`   状态码: ${response.status}`);
      console.log(`   Location: ${location}`);

      if (location.includes('cloudflareaccess.com') || location.includes('cdn-cgi/access/login')) {
        console.log('\n❌ 当前 Appwrite 域名被 Cloudflare Access 保护，浏览器端上传会直接失败。');
        console.log('   解决方向:');
        console.log('   1. 给前端访问路径放行 /v1/*');
        console.log('   2. 或改用未受保护的 Appwrite 域名');
        console.log('   3. 前端项目不需要也不应该使用 Appwrite API Key\n');
      }
